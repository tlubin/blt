...
void TreeInsertHelp(rb_red_blk_node* z) {
  ...
  while( x != nil) {
    y=x;
    if (1 == compare(x->key,z->key)) { /* x.key > z.key */
      x=x->left;
    } else { /* x,key <= z.key */
      x=x->right;
    }
  ...
  z->parent=y;
  if ( (y == tree->root) ||
       (1 == compare(y->key,z->key))) { /* y.key > z.key */
    y->left=z;
  } else {
    y->right=z;
  }
...
/* XXX: original code was :     if (x->parent == x->parent->parent->left) { */
    if ( false && x->parent == x->parent->parent->left) {
